...
unsigned char lyrics_farewell[] = {
  SYSEX_BEGIN, 0x0A, 0x00,
  0x4b,                         // ほ
  0x29, 0x71, 0x43, 0x48,       // たるのひ
  0x05, 0x00, 0x70, 0x64,       // かありま
  0x32, 0x43, 0x6d, 0x02,       // どのゆう
  0x06,                         // き
...
int main(int argc, char** argv)
{
	...
	while (1) {
#endif  // INFINIT_LOOP
        if (notes_farewell[0].pitch != PITCH_R) {
            SEND_PITCH_COMMAND(notes_farewell[0].pitch);
        }
		midiout->sendMessage( note_on_cmd, sizeof( note_on_cmd ) );
		for (int i = 1; i < sizeof (notes_farewell) / sizeof (notes_farewell[0]); i++ ) {
